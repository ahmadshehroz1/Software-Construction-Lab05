...
public class ExtractTest {
    ...
    @Test
    public void getMentionedUsers_SameUserMultipleTimes_ReturnsUniqueUser() {
        Tweet tweet1 = new Tweet(1, "testUser", "Hello @user123 @user123", d1);
        Tweet tweet2 = new Tweet(2, "anotherUser", "Just saying hi to @user123 again!", d2);
        Set<String> mentionedUsers = Extract.getMentionedUsers(Arrays.asList(tweet1, tweet2));
        Set<String> expected = new HashSet<>(Arrays.asList("user123"));
        assertEquals("expected mentioned users", expected, mentionedUsers);
    }
    @Test
    public void getTimespan_IdenticalTimestamps_ReturnsSameStartAndEnd() {
        Instant identicalTimestamp = Instant.parse("2016-02-17T10:00:00Z");
        Tweet tweet1 = new Tweet(1, "user1", "First tweet", identicalTimestamp);
        Tweet tweet2 = new Tweet(2, "user2", "Second tweet", identicalTimestamp);
        Timespan timespan = Extract.getTimespan(Arrays.asList(tweet1, tweet2));
        assertEquals("expected start", identicalTimestamp, timespan.getStart());
        assertEquals("expected end", identicalTimestamp, timespan.getEnd());
    }
}